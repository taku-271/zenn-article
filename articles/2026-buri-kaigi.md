...
2025年の振り返りと2026年、AIとどう向き合ってエンジニアリングしていくかという内容でした。
以下、内容をまとめました。
## 2025年の振り返り
### AIコーディングに対する振り返り
2025年に`Vibe Coding`というものが出てきました。これは**70%の問題を解決できるが、30%の問題を解決することが難しい**と言われています。また、AIはもっともらしい出力を出すので、レビューコストが上がっています。さらに、`Vibe Coding`で作成したソフトウェアは、自分が欲しいものを作ることに対しては有効だが、仕様の不確定さなどから他者へ提供するソフトウェアにはまだ向いていません。
...
AIは今までにはない非決定性という性質を持っています。つまり、AIに決定性や再現性を求めることは難しいということになります。これは、LLMの核心は単に次の単語を予測する（マルコフ連鎖）モデルであり、その確率性をなくすことはできないためです。そのため、**ハルシネーションはバグではなくAI/LLMの仕様**と言えます。

### AIの非決定性をどうするか
非決定性は仕様なので、AIにいうことを聞かせようとするアプローチには限界があります。うまくいかずにハラスメントになるという経験をした方々も多いと思います。そのため、AIがいうことを聞かなくても問題ないようなアプローチを行うことで、工学的にも精神的にも良いアクションを取ることができます。
このように、今後はソフトウェア開発で「部分的に確率的に間違っていても仕組みで気づけるプロセス」を考えないといけません。
...
これは、大規模開発のアーキテクトが直面してきた問題に似ています。大人数のエンジニアが同じコードを編集するため、この全てのコードを理解している人はいないと思います。
...
例えば、オライリーの`Architecture As Code`に載っている、`ADL: Architecture Definition Language`を疑似コードとして作成し、AIに正しいアーキテクチャの判断基準として読み込ませておくことで簡単に適応度関数を生成することができます。疑似コードから実行可能なプログラムにすることでCIに組み込むことも可能になります。

### ソフトウェア開発で重点を置く部分
ソフトウェア開発をする上で、従来は理解容易性と変更容易性はトレードオフの関係でした。突き詰めていくと、最終的にどちらかに枝分かれをするはずです。
AIが出てきた今後、コードの変更者がAIに置き換わることで、変更に対する許容量が大幅に向上します。そのため、そのどちらに対して投資をするかと言われたら理解容易性への投資を大切にする方が良くなります。変更用意性はAIに24時間365日で働いて貰えば賄えるからです。
...
例えば、重要ではあるが非緊急のタスクは、リソースが限られていると後回しにされてしまいます。しかし、AIがここにコミットすることで、今まで後回しにしていたことを進めることができます。
また、自動テストをAIに実装してもらうことで、自動テストへのコストが大幅に減ります。従来はできる限り少ないコード、テスト量で多くのケースを拾うことがベストプラクティスでしたが、AIに自動テストを任せることで、実装コストの許容量が大幅に増加し、カバレッジ率100%のテストケースがアンチパターンではなくなるかもしれません。

### AIの知識量を使いこなす
「LLMには知識はあるが知恵がない」という言葉があります。
AI/LLMには膨大な知識量がありますが、それらの知識を引き出すスキル（知恵）は持っていません。いかに私たちがその知識を引き出せるか、的確にその内容を伝え、使ってもらうのかが大事になります。
そのため、私たちも学習を怠ってはいけないということですね。
...
今後大切になるのは、AIを使う際の仕組みづくり、いかにAIと共同できるか、うまく使うかが鍵になると思います。このセッションで学んだことを元に2026年はAIを活かせるようにしたいです。
...
そうすると、シニアエンジニアはAIのコードに対してレビューを行う必要があり、責任の所在を意識しなければいけない、コーディングエンジニアに価値がなくなってしまう、エンジニアが自身のコードを保証できるのかなどの問題が発生します。
AIを使用したとしても、コードの責任はあくまでユーザーにあります。例えCodeRabbitなどのAIエージェントがOKを出した内容でもそのコードの責任はユーザーが取らなければいけません。あくまで、CodeRabbitはエンジニアがレビューする前の1次レビューとして使うべきです。
...
レビューの自動割り当てとは、レビュアーを順番に割り当て、負担をバランシングすることです。これにより、負担が集中しない、属人化を抑えられるなどのメリットがあります。最近だとGitHubの機能に自動割り当て機能があります。そのようなVCSなどの機能に頼るとすぐ導入できると思います。
また、レビューの視点、無視すること、確認することを定義し一律の基準を設けることも大事です。また、レビュアーだけでなくレビューイのためのガイドラインを策定することも重要です。
例えば、Googleが公開している"[Google Engineering Practices Documentation](https://fujiharuka.github.io/google-eng-practices-ja/)"などのガイドラインを参考に自社の基準を作成するとガイドライン作成のハードルが低くなっておすすめです。

## レビューの正しい道標
コードレビューに対して"目的"と"ゴール"を明確にし、時間と労力を浪費しないようにする必要があります。
そのためには、以下のような対策が重要になります。
- 500行以下の小さなプルリクエストの作成。
- レビュー項目の優先度の設定（アーキテクチャ、DBスキーマは優先度高めでみるなど）。
- レビュー時間の確保。
- CodeRabbitなどのAIコードレビューの導入。
...
開始後、問題が配られ、TDDのRed Green Refactorという手順で順々に進めていきました。久しぶりのPHPでしたが、ペアプロの相方の方がとてもリードしてくださり、難なく進めることができました。

## 感想
やってみて、難しかったが、とても楽しかったです。特に、コードの実態がない中でテストコードを書くという部分を特訓することができ、一度TDDでプロジェクトを動かしてみたいと思いました。
しかし、テストケースやテストコードの作成については、網羅することや実態がない中でテストコードを書くなど、結構なスキルが必要だなと感じました。
TDD BootCamp、会社内でも展開したいなと思っています。

# 全体の感想
初めてのBuriKaigiでした。とても楽しく、基調講演やCodeRabbitの話、TDDなどとても多くの収穫がありました。また、様々なエンジニアと交流することで自分自身のモチベーションを向上させることができ、本当に参加して良かった会だと思います。
とても有意義なセッションを聞き、私も人を惹きつけるようなセッションを行いたいと思いました。今年はたくさん登壇して自分自身のセッションに対するスキルを向上させたいと思います。
本当はDay 2も行きたかったのですが、会社の予定とかぶってしまったのでいけませんでした。来年こそは全日参加したいです！
