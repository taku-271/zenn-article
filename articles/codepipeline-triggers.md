...
Code兄弟を使用した、CI/CDがあるインフラを構築することが多くなってきました。
また、モノレポになっているので、インフラやアプリケーションが同じリポジトリに存在します。

Code Connectionsのトリガーを実装しましたが、インフラのコードを変えただけでもCode Pipelineが発火してしまうと言う問題があります。
Code Pipelineのトリガーにはパスフィルターが実装されていますが、CDK（L2コンストラクト）からはそれが設定できません。

コストや運用優秀性の観点から、パスフィルターができないかを試行錯誤した結果を綴ります。

# 解決方法
CDKで作成したL2のCode Pipelineコンストラクタに対して、CloudFormationプロパティを上書きするようにすれば解決できます。
:::message
...
これは何かというと、L2コンストラクトに定義されていないそのL1コンストラクトにアクセスできるプロパティのことです。
